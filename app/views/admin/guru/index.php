                        ...
                        <tr class="hover:bg-blue-100 cursor-pointer text-gray-700">
                            ...
                            <td class="p-3">
                                <div class="flex space-x-2">
                                    <!-- Tombol Edit -->
                                    <button onclick="editGuru('<?= $guru['id_guru'] ?>', '<?= $guru['id_user'] ?>', '<?= addslashes($guru['nama_guru']) ?>')"
                                    ...
                                    <button class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 rounded-lg text-sm p-2"
                                        onclick="confirmDelete('guru/delete', <?= $guru['id_guru'] ?>)">
                                        <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            fill="currentColor" viewBox="0 0 24 24">
                                            <path fill-rule="evenodd"
    ...
    <div class="relative w-full max-w-4xl max-h-full">
        <div class="relative bg-white rounded-lg shadow-lg">
            ...
            <div class="modal-body">
                <form action="<?= BASEURL_ADMIN ?>/guru/quran" method="POST" id="form" class="p-6">
                    <input type="hidden" name="id" id="id">

                    <!-- santri -->
                    <div class="mb-4">
                        <label for="guru" class="block mb-2 text-sm font-medium">Nama Guru</label>
                        <button id="dropdownGuruButton" data-dropdown-toggle="dropdown-guru"
                            ...
                            type="button">
                            <span id="selected-guru">Pilih Guru</span>
                        ...
                        <div id="dropdown-guru"
                            class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                            <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownGuruButton">
                                <?php foreach ($data['guru_user'] as $nama_guru): ?>
                                    <li>
                                        <a href="#" class="block px-4 py-2 hover:bg-gray-100"
                                            onclick="selectGuru('<?= $nama_guru['id_user'] ?>', '<?= $nama_guru['nama'] ?>')">
                                            <?= $nama_guru['nama'] ?>
                                        </a>
                                    </li>
                                <?php endforeach; ?>
...
<script>
    function editGuru(id_guru, id_user, nama_guru) {
        document.getElementById('modalLabel').innerText = 'Ubah Data Guru';
        document.getElementById('id').value = id_guru;
