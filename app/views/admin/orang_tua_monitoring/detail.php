    ...
    <div class="p-5 bg-gray-100 shadow-lg rounded-lg">
        <div class="flex justify-between items-center mb-4">
            ...
            <?php if (!isset($data['is_orang_tua']) || !$data['is_orang_tua']): ?>
                <a href="<?= BASEURL; ?>/admin/Orang_tua_monitoring"
        ...
        <?php elseif (isset($data['is_guru']) && $data['is_guru']): ?>
            <div class="p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 border border-blue-300" role="alert">
                ...
                <div class="pl-7">Di sini Anda dapat melihat detail perkembangan hafalan santri dari orang tua yang dipilih.</div>
            </div>
        <?php endif; ?>
                    ...
                    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="santriTabs" role="tablist">
                        <?php foreach ($data['santri_list'] as $index => $santri): ?>
                            <li class="mr-2" role="presentation">
                                <button class="inline-block p-4 border-b-2 rounded-t-lg <?= $index === 0 ? 'text-blue-600 border-blue-600 active' : 'hover:text-gray-600 hover:border-gray-300 border-transparent' ?>"
                                    id="santri-tab-<?= $santri['id_santri']; ?>"
                                    data-tabs-target="#santri-<?= $santri['id_santri']; ?>"
                                    type="button"
                                    role="tab"
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-blue-500">
                                        ...
                                        <p class="text-2xl font-bold text-blue-900"><?= $santri['laporan']['total_pertemuan'] ?? 0; ?></p>
                                    </div>
                                    <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-green-500">
                                        <p class="text-gray-500 text-sm">Total Hafalan Ayat</p>
                                        <p class="text-2xl font-bold text-green-700"><?= $santri['laporan']['total_hafalan_ayat'] ?? 0; ?></p>
                                    </div>
                                    <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-purple-500">
                                        <p class="text-gray-500 text-sm">Total Hafalan</p>
                                        <p class="text-2xl font-bold text-purple-700"><?= $santri['laporan']['total_hafalan'] ?? 0; ?></p>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <tbody>
                                                ...
                                                <?php foreach ($santri['detail_hafalan'] as $hafalan): ?>
                                                    <tr class="bg-white border-b hover:bg-gray-50">
                                                        ...
                                                        <td class="px-6 py-4"><?= htmlspecialchars($hafalan['keterangan'] ?? '-'); ?></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                                        </table>
